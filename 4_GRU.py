...
if st.button("TRAIN THE MODEL"):
    ...
    torch.save(model.state_dict(), 'gru_model.pht')
    file_path = "GRU_array_data.csv"
    np.savetxt(file_path, predicted_profit, delimiter=",")
    st.write("MODEL IS TRAINED AND SAVED")
    
# Load the model
loaded_model = GRU(input_size, hidden_size, num_layers, output_size).to(device)
loaded_model.load_state_dict(torch.load('gru_model.pht', map_location=torch.device('cpu')))
loaded_model.eval()
